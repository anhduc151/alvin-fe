...
if [[ ! -f  "$SCRIPT_ROOT_DIR/.env" ]]
then
  echo "ERROR: Not found .env file in root folder project at: ${SCRIPT_ROOT_DIR}"
  echo "Please create .env from .env.exmple file in root folder project at: ${SCRIPT_ROOT_DIR}"
  exit 1
fi
...
then
  set -o allexport
	eval $(cat "$SCRIPT_ROOT_DIR/.env" | sed -e '/^#/d;/^\s*$/d' -e 's/\(\w*\)[ \t]*=[ \t]*\(.*\)/\1=\2/' -e "s/=['\"]\(.*\)['\"]/=\1/g" -e "s/'/'\\\''/g" -e "s/=\(.*\)/='\1'/g")
	set +o allexport
else
  echo "ERROR: Not found .env file in root folder project at: ${SCRIPT_ROOT_DIR}"
  echo "Please create .env from .env.exmple file in root folder project at: ${SCRIPT_ROOT_DIR}"
...
DOCKER_BUILDKIT=0 DOCKER_SCAN_SUGGEST=false docker-compose -f infra/docker-compose.local.yml build --no-cache
